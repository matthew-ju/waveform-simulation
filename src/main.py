...
config_name = os.path.join(project_root, "configs", "dlopy.yml")
...
#****
# Load velocity maps from the data directory
maps_dir = os.path.join(project_root, "data", "maps")
MAP10 = np.loadtxt(os.path.join(maps_dir, 'R.gv.10.txt'), skiprows=1)
MAP15 = np.loadtxt(os.path.join(maps_dir, 'R.gv.15.txt'), skiprows=1)
MAP20 = np.loadtxt(os.path.join(maps_dir, 'R.gv.20.txt'), skiprows=1)
MAP25 = np.loadtxt(os.path.join(maps_dir, 'R.gv.25.txt'), skiprows=1)
MAP30 = np.loadtxt(os.path.join(maps_dir, 'R.gv.30.txt'), skiprows=1)
MAP35 = np.loadtxt(os.path.join(maps_dir, 'R.gv.35.txt'), skiprows=1)
MAP40 = np.loadtxt(os.path.join(maps_dir, 'R.gv.40.txt'), skiprows=1)
...
def loopstations(sta_value):
    ...
    loc1 = os.path.join(project_root, "outputs", time1 + "_to_" + time2, "station_runs", sta)

    # create output directory
    if not os.path.exists(loc1):
        os.makedirs(loc1)
    
    # Ensure trailing slash for legacy code compatibility if needed, 
    # but it's better to use os.path.join inside the code.
    loc1_folder = loc1 + os.sep
    ...
    ## Load files
    R1phi = loadtxt(os.path.join(loc1, 'R1phi'))
    R1cc = loadtxt(os.path.join(loc1, 'R1cc'))

    R2phi = loadtxt(os.path.join(loc1, 'R2phi'))
    R2cc = loadtxt(os.path.join(loc1, 'R2cc'))
    ...
    plt.grid()
    plt.savefig(os.path.join(loc1, f"{sta}.v2.cluster.pdf"))
